...
@Suppress("MemberVisibilityCanBePrivate") //java interop nullability fix.
class LoggerPrintStream( //could make internal but you never know
	val logStream: (Thread, StackTraceElement, Instant, String) -> Boolean
) : PrintStream(NullOutputStream()) {
	
	private fun write(content: String): Boolean = logStream(
		currentThread(),
		//[0] = Thread#getStackTrace
		//[1] = LoggerPrintStream#write
		//[2] = LoggerPrintStream#print(ln)
		//[3] = Caller#method
		currentThread().stackTrace[3],
		now(),
		content.removeSuffix(lineSeparator())
	)
